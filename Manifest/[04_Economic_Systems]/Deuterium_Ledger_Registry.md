...
* **Lore Integration**: The economy is a physical reflection of world stability. When energy supply is abundant, the Watt is stable; when it is hoarded by the Consortium, the value of the Watt spikes, creating an artificial desperation.

## 2. LOGIC ARCHITECTURE
To maintain performance and simulation depth, the economy uses a decoupled, three-tick delay pipeline to prevent CPU bottlenecking.
...
2. **Purity Harvesting**: The engine calculates the `VoidMultiplier`. If Young Sir harvests Deuterium in negative light (`LightIntensity < 0`), the engine applies a multiplier based on the absolute value of darkness.
3. **Snapshot Update**: Street vendors query the 3-tick lag buffer for pricing.
4. **Arbitrage**: Elite black-market traders bypass the buffer, querying the core registry directly.

## 5. RELATIONAL DEPENDENCIES
* **Physics**: Prices fluctuate based on asset values tracked in the `Dynamic_Movement_Matrix`.
...
* Standard vendors are FORBIDDEN from real-time core registry polling.
